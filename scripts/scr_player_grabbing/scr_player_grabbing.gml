function scr_player_grabbing()
{
    ...
    {
        movespeed = 0;
        sprite_index = get_charactersprite("spr_grabbing");
    }
    
    if (move != 0)
    {
        if (grounded)
            sprite_index = get_charactersprite("spr_grabbingmove");
    ...
    if (key_jump && (grounded && (!key_down && !key_attack)))
    {
        vsp = -9;
        image_index = 0;
        sprite_index = get_charactersprite("spr_grabbingjump");
    }
    
    if (sprite_index == get_charactersprite("spr_grabbingjump") && floor(image_index) == 2)
        sprite_index = get_charactersprite("spr_grabbingfall");
